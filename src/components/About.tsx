export default function About() {
  return (
    <section id="over-ons" className="relative py-28 md:py-36 section-about overflow-hidden" aria-labelledby="about-heading">

      {/* Diagonal decorative element — top right corner glow */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] pointer-events-none" aria-hidden="true"
      ...
      <div className="absolute left-0 top-16 bottom-16 w-px hidden lg:block" aria-hidden="true"
        style={{ background: "linear-gradient(to bottom, transparent, rgba(181,98,42,0.3) 30%, rgba(196,144,46,0.4) 50%, rgba(181,98,42,0.3) 70%, transparent)" }} />
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <div className="reveal">
            <p className="label-caps mb-4" style={{ color: "#B5622A" }}>Over de Kloeg</p>

            <h2 id="about-heading" className="font-display text-4xl md:text-5xl font-bold leading-tight mb-6"
              style={{ color: "#2A1E12" }}>
              Vroeger De Wandeler,{" "}
              <em className="italic" style={{ color: "#A8401E" }}>nu De Kloeg.</em>
            </h2>

            {/* Copper + gold decorative rule */}
            ...
            <div className="space-y-5 font-body leading-relaxed text-[1.05rem]" style={{ color: "#4A3320" }}>
              ...
              <p>
                ...
                <em className="italic font-semibold" style={{ color: "#6B1A2A" }}>kanen, knagen, genieten.</em>
              </p>
            </div>

            {/* Stats row with colour-coded numbers */}
            <div className="mt-10 grid grid-cols-3 gap-6">
              {[
                ...
                <div key={num} className="text-center group">
                  <p className="font-display text-3xl font-black leading-none mb-1.5 transition-transform duration-300 group-hover:scale-110"
                    style={{ color }}>{num}</p>
                  <p className="label-caps text-[10px] leading-snug whitespace-pre-line" style={{ color: "#7A5C40" }}>{label}</p>
                </div>
              ))}
            </div>
          </div>

          {/* ── Right: Deelder quote card ──────────────────────────── */}
          <div className="reveal reveal-delay-2">
            {/* Card with layered dark + grain + warm glow */}
            <div className="relative rounded-sm overflow-hidden card-dark p-10 md:p-12">
              ...
              {/* Decorative corners */}
              <div className="absolute top-5 left-5 w-7 h-7 border-t border-l" aria-hidden="true"
                style={{ borderColor: "rgba(196,144,46,0.35)" }} />
              <div className="absolute bottom-5 right-5 w-7 h-7 border-b border-r" aria-hidden="true"
                style={{ borderColor: "rgba(196,144,46,0.35)" }} />

              {/* Attribution */}
              <p className="label-caps text-[10px] mb-5" style={{ color: "rgba(196,144,46,0.5)" }}>— Jules Deelder</p>
